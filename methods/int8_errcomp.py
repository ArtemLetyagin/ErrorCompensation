...
def fsdp_int8_error_feedback_hook(
    state: FSDPInt8EFCommState,
    grad: torch.Tensor,
    ...
    # residual stores the quantization error that will be added next time.
    state.residuals[hook_id] = (corrected_grad - deq).detach()
    ...
    state.add_record(
        "int8_ef_all_gather_reduce_scatter",
        msg_size_bytes_per_rank * world_size,
        latency_ms,
    )

def register_fsdp_int8_error_feedback_hook(model, rank, world_size):
    state = FSDPInt8EFCommState(
    ...
    model.register_comm_hook(state, fsdp_int8_error_feedback_hook)
    return state