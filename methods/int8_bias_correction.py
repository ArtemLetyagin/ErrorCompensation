...
@dataclass
class FSDPInt8BiasCorrectionCommState:
...
def fsdp_int8_bias_correction_hook(
    state: FSDPInt8BiasCorrectionCommState,
    grad: torch.Tensor,
    ...
    bias = state.biases.get(hook_id)

    if bias is None:
        bias = torch.tensor(0.0, device=grad.device, dtype=torch.float32)

    corrected_grad = grad + bias

    q, scale = quantize_int8_symmetric(corrected_grad)
    deq = dequantize_int8_symmetric(q, scale)

    # Bias correction update:
    # bias stores the mean quantization error and adds it on the next call.
    state.biases[hook_id] = (corrected_grad - deq).mean().detach()
    ...
    if output is None:
        ...
        state.add_record(
            "int8_bias_correction_all_gather_all_reduce",
    ...
    for q_i, scale_i in zip(q_list, scale_list):
        reduced_shard += dequantize_int8_symmetric(q_i[start:end], scale_i)
    ...
    state.add_record(
        "int8_bias_correction_all_gather_reduce_scatter",
        msg_size_bytes_per_rank * world_size,
        latency_ms,
    )


def register_fsdp_int8_bias_correction_hook(model, rank, world_size):
    state = FSDPInt8BiasCorrectionCommState(
    ...
    model.register_comm_hook(state, fsdp_int8_bias_correction_hook)
    return state